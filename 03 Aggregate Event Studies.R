...
# log sales
ES_rev = feols(data = base_df, log(total_rev_per_cap) ~ ref_num + temp_mean | fips + year + month, cluster = base_df$fips)
...
ggplot(data = ES_results, aes(x = time, y = estimate))+
  ...
  geom_point(aes(x = -2, y = 0), fill = "white", shape = 21, size = 2)+
  theme(axis.title = element_text(size = 25), axis.text = element_text(size = 20))+
  labs(title = " ", x = "Weeks", y = "Log Total Revenue per 100K Residents")
...
# log price
ES_price = feols(data = base_df, log(price) ~ ref_num + temp_mean | fips + year + month, cluster = base_df$fips)
...
ggplot(data = ES_price_results, aes(x = time, y = estimate))+
  ...
  geom_point(aes(x = -2, y = 0), fill = "white", shape = 21, size = 2)+
  theme(axis.title = element_text(size = 25), axis.text = element_text(size = 20))+
  labs(title = " ", x = "Weeks", y = "Log Average Price")
...
ES_vol <- feols(data = base_df, log(total_vol/CENSUS2010POP*100000)~ ref_num + temp_mean | fips + year + month, cluster = base_df$fips)
...
ggplot(data = ES_vol_results, aes(x = time, y = estimate))+
  ...
  geom_point(aes(x = -2, y = 0), fill = "white", shape = 21, size = 2)+
  theme(axis.title = element_text(size = 25), axis.text = element_text(size = 20))+
  labs(title = " ", x = "Weeks", y = "Log Total Ounces Sold per 100K Residents")
...
ts_results <- make_ES_table(ES_ts, "Tropical Storm")


# Minor Hurricanes

cat1_df <- get_event_data(scanner, "none", c(0,100), c(64,96))
ES_cat1 = feols(data = cat1_df, total_rev_per_cap ~ ref_num + temp_mean | fips + year + month, cluster = cat1_df$fips)
cat1_results <- make_ES_table(ES_cat1, "Minor Hurricane")
...
major_results <- make_ES_table(ES_major, "Major Hurricane")
...
# Log Total Rev by Category-----------------------------------------------------

# Tropical storm or weaker

ts_df <- get_event_data(scanner, "none", c(0,100), c(0,64))
ES_ts = feols(data = ts_df, log(total_rev_per_cap) ~ ref_num + temp_mean | fips + year + month, cluster = ts_df$fips)
ts_results <- make_ES_table(ES_ts, "Tropical Storm")
...
cat1_df <- get_event_data(scanner, "none", c(0,100), c(64,96))
ES_cat1 = feols(data = cat1_df, log(total_rev_per_cap) ~ ref_num + temp_mean | fips + year + month, cluster = cat1_df$fips)
cat1_results <- make_ES_table(ES_cat1, "Minor Hurricane")

# major 

major_df <- get_event_data(scanner, "none", c(0,100), c(96,200))
ES_major = feols(data = major_df, log(total_rev_per_cap) ~ ref_num + temp_mean | fips + year + month, cluster = major_df$fips)
major_results <- make_ES_table(ES_major, "Major Hurricane")
...
ggplot(data = all_results, aes(x = time, y = estimate,color = id))+
  ...
  labs(title = " ", x = "Weeks", y = "Log Total Revenue per 100K Residents", color = "Storm Class")
